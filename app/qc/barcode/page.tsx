...
import { Search, Loader2, QrCode, Filter, RefreshCw, Hash, FileText, CheckCircle, Package, AlertTriangle, X, Download } from "lucide-react";
...
export default function BarcodePage() {
  ...
  const uniqueMesin = Array.from(new Set(allData.map(d => d.nomor_mc).filter(Boolean))).sort();
  const uniqueDesign = Array.from(new Set(allData.map(d => d.design_id).filter(Boolean))).sort();
  const uniquePotongan = Array.from(new Set(allData.map(d => String(d.potongan_ke)).filter(Boolean))).sort((a,b) => Number(a) - Number(b));
  ...
  const filteredData = allData.filter((batch) => {
    ...
    if (filters.potongan_ke && String(batch.potongan_ke) !== filters.potongan_ke) return false;
    return true;
  });
  ...
  const handleDownload = async () => {
    ...
    try {
      const dataUrl = await toPng(node, { quality: 1, backgroundColor: '#ffffff', pixelRatio: 2 });
    ...
    <div className="w-full h-full pb-20 animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium mt-1">Cetak barcode hasil akhir inspeksi dan mending per batch.</p>
        </div>
      </div>
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6 relative z-10">
        <div className="flex flex-col sm:flex-row gap-4 items-end">
          <div className="flex flex-col gap-1 w-full sm:w-1/3">
             <label className="text-[10px] font-bold text-slate-500 uppercase">Nomor Mesin</label>
             <select
          ...
          <div className="flex flex-col gap-1 w-full sm:w-1/3">
             <label className="text-[10px] font-bold text-slate-500 uppercase">Design ID</label>
             <select
        ...
        {(filters.nomor_mc || filters.design_id || filters.potongan_ke) && (
          <div className="mt-3 flex items-center justify-between">
            <span className="text-xs text-slate-500 font-medium">Menampilkan {filteredData.length} dari {allData.length} batch</span>
            <button
              onClick={() => setFilters({ nomor_mc: "", design_id: "", potongan_ke: "" })}
      ...
      <div className="space-y-4 relative z-0">
        {isLoading ? (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 flex flex-col items-center justify-center text-center animate-fadeIn">
            ...
            <h3 className="text-lg font-bold text-slate-700 mb-1">Memuat Data...</h3>
            <p className="text-sm text-slate-500">Mengambil batch yang sudah di-mending</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 flex flex-col items-center justify-center text-center animate-fadeIn">
            ...
            <h3 className="text-lg font-bold text-slate-700 mb-1">{allData.length === 0 ? "Belum Ada Data Mending" : "Tidak Ditemukan"}</h3>
            <p className="text-sm text-slate-500 max-w-md mx-auto">{allData.length === 0 ? "Belum ada batch yang sudah selesai proses mending." : "Coba ubah filter pencarian Anda."}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredData.map((batch, i) => (
              <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 hover:shadow-md transition-all group flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-xs font-bold text-slate-400">{batch.tgl}</span>
                </div>
                
                <h3 className="text-lg font-black text-slate-800 mb-4">{batch.design_id || "-"}</h3>
                
                <div className="grid grid-cols-2 gap-3 mb-5 flex-1">
                  <div className="bg-slate-50 rounded-lg p-2.5 border border-slate-100">
                    <span className="block text-[10px] font-bold text-slate-400 uppercase">Panel Ke</span>
                    <span className="block text-sm font-black text-slate-700">PCS {batch.pcs_index}</span>
                  </div>
                  <div className="bg-slate-50 rounded-lg p-2.5 border border-slate-100">
                    <span className="block text-[10px] font-bold text-slate-400 uppercase">Berat Kain</span>
                    <span className="block text-sm font-black text-slate-700">{batch.berat_kain} Kg</span>
                  </div>
                </div>
      ...
      {isModalOpen && selectedBatch && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 print:p-0 print:items-start print:bg-white">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm print:hidden" onClick={() => setIsModalOpen(false)} />
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200 max-h-[90vh] print:shadow-none print:w-full print:max-w-none print:h-screen print:max-h-none print:rounded-none">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 print:hidden">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800">Cetak Barcode</h2>
                  <p className="text-xs text-slate-500 font-medium mt-0.5">Generate QR Code Informasi QC</p>
                </div>
              </div>
            ...
            <div className="p-6 overflow-y-auto print:p-0 print:overflow-visible">
              ...
              <div className="mb-6 pb-6 border-b border-slate-100 print:hidden">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Masukkan Grade Final</label>
                <select
                  value={gradeInput}
                ...
                <p className="text-xs text-slate-500 mt-2">Grade ini akan dimasukkan ke dalam data QR Code.</p>
              </div>

              {/* Printable Area */}
              <div id="print-area" className="flex flex-col items-center justify-center p-4 bg-white">
                <div className="border-4 border-slate-900 p-6 rounded-3xl flex flex-col items-center gap-4 bg-white shadow-sm print:shadow-none print:border-black print:rounded-none">
                  <div className="text-center mb-2">
                    <h3 className="font-black text-2xl text-slate-900 tracking-tight">DJI - QC PASS</h3>
                    <p className="text-sm font-bold text-slate-600">GRADE {gradeInput}</p>
                  </div>
                  ...
                  <div className="w-full mt-4 space-y-1.5 border-t-2 border-dashed border-slate-200 pt-4">
                     <div className="flex justify-between text-xs font-semibold text-slate-700">
                        <span>Desain:</span>
                        <span className="font-bold text-slate-900">{selectedBatch.design_id}</span>
                     </div>
                     <div className="flex justify-between text-xs font-semibold text-slate-700">
                        <span>No Customer:</span>
                        <span className="font-bold text-slate-900">{selectedBatch.no_customer || "-"}</span>
                     </div>
                     <div className="flex justify-between text-xs font-semibold text-slate-700">
                        <span>Mesin/Pot:</span>
                        <span className="font-bold text-slate-900">{selectedBatch.nomor_mc} / {selectedBatch.potongan_ke}</span>
                     </div>
                     <div className="flex justify-between text-xs font-semibold text-slate-700">
                        <span>Tanggal:</span>
                        <span className="font-bold text-slate-900">{selectedBatch.tgl || "-"}</span>
                     </div>
                     <div className="flex justify-between text-xs font-semibold text-slate-700">
                        <span>Berat:</span>
                        <span className="font-bold text-slate-900">{selectedBatch.berat_kain} Kg</span>
                     </div>
                     <div className="flex justify-between text-xs font-semibold text-slate-700">
                        <span>PCS Ke:</span>
                        <span className="font-bold text-slate-900">{selectedBatch.pcs_index}</span>
                     </div>
                  </div>
                </div>
              </div>
