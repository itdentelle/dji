...
const DeltaBadge = ({ delta, isLowerBetter = false }: { delta: { value: number; type: "up" | "down" | "flat" }; isLowerBetter?: boolean }) => {
  if (delta.type === "flat" || delta.value === 0) {
    return <span className="text-[10px] text-slate-400 font-extrabold ml-1.5">0.0% vs periode lalu</span>;
  }
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-extrabold border ${colorClass} ml-1.5`}>
      {arrow} {sign}{delta.value}% vs periode lalu
    </span>
  );
};
...
export default function DashboardPage() {
  ...
  const [dashboardViewType, setDashboardViewType] = useState<"OEE" | "CLASSIC">("CLASSIC");
  ...
  const [chartGradeFilter, setChartGradeFilter] = useState<"ALL" | "GRADE_A" | "GRADE_B" | "BS" | "UNGRADED">("ALL");
  ...
  const calculateStatsForDataset = (dataset: Transaction[], gradeFilter: string) => {
    ...
    productionOnly.forEach(item => {
      if (item.panel_no !== undefined && item.panel_no !== null && !isNaN(Number(item.panel_no)) && (item.hasil_meter || 0) === 0) {
    ...
    const totalProduksiMeter = uniqueHeaders.reduce((acc, curr) => acc + (parseFloat(curr.hasil_meter as any) || 0), 0);
    const totalTarget = uniqueHeaders.reduce((acc, curr) => acc + curr.target_pcs, 0);
    const totalHasilPcs = productionOnly.reduce((acc, curr) => acc + curr.hasil_pcs, 0); // Keep raw sum since it's detail-level pcs
    ...
    const uniqueGradeScopedHeadersMap = new Map<string, typeof gradeScoped[0]>();
    ...
    const uniqueGradeScopedHeaders = Array.from(uniqueGradeScopedHeadersMap.values());
    const totalDowntimeDetik = uniqueGradeScopedHeaders.reduce((acc, curr) => acc + (curr.total_downtime_detik || 0), 0);
    const totalDetikKerjaEfektif = Math.max(0, totalDetikTersedia - totalDowntimeDetik);
    const availability = totalDetikTersedia > 0 ? (totalDetikKerjaEfektif / totalDetikTersedia) * 100 : 0;

    // Performance Rate
    const performance = totalTarget > 0 ? (totalHasilPcs / totalTarget) * 100 : 0;
  ...
  const currentHalfFilteredTransactions = useMemo(() => {
    ...
    return result;
  }, [transactions, dateFilteredTransactions, dateRangeMode, selectedOperators]);
  ...
  const deltas = useMemo(() => {
    const calculateDelta = (curr: number, prev: number): { value: number; type: "up" | "down" | "flat" } => {
      if (prev === 0) {
    ...
    const calculatePointDelta = (curr: number, prev: number): { value: number; type: "up" | "down" | "flat" } => {
      const diff = curr - prev;
      return {
    ...
    const statsForDelta = calculateStatsForDataset(modeScopedDelta, chartGradeFilter);
    const statsForPrev = calculateStatsForDataset(modeScopedPrev, chartGradeFilter);

    return {
      oee: calculatePointDelta(statsForDelta.oee, statsForPrev.oee),
      availability: calculatePointDelta(statsForDelta.availability, statsForPrev.availability),
      performance: calculatePointDelta(statsForDelta.performance, statsForPrev.performance),
      quality: calculatePointDelta(statsForDelta.quality, statsForPrev.quality),
      totalProduksi: calculateDelta(statsForDelta.totalProduksi, statsForPrev.totalProduksi),
      totalProduksiMeter: calculateDelta(statsForDelta.totalProduksiMeter, statsForPrev.totalProduksiMeter),
      countMasalah: calculateDelta(statsForDelta.countMasalah, statsForPrev.countMasalah),
    };
  }, [currentHalfFilteredTransactions, prevFilteredTransactions, chartGradeFilter, metricMode]);
  ...
  const leaderboard = useMemo(() => {
    ...
    dateFilteredTransactions.forEach(item => {
      if (item.is_production && item.header_id) {
        if (!operatorMap[item.nama_operator]) {
          operatorMap[item.nama_operator] = { headers: new Set(), group: item.group || "Tanpa Group" };
        }
        operatorMap[item.nama_operator].headers.add(item.header_id);
      }
    });
    const topOperators = Object.entries(operatorMap)
      .map(([name, data]) => ({ name, pcs: data.headers.size, group: data.group }))
      .sort((a, b) => b.pcs - a.pcs)
      .slice(0, 5);

    // 2. Top Machines (ranked by total unique panels)
    const machineMap: Record<string, { headers: Set<string>; downtimeHeaders: Map<string, number> }> = {};
    dateFilteredTransactions.forEach(item => {
      if (item.is_production && item.header_id) {
        if (!machineMap[item.mesin_id]) {
          machineMap[item.mesin_id] = { headers: new Set(), downtimeHeaders: new Map() };
        }
        machineMap[item.mesin_id].headers.add(item.header_id);
        machineMap[item.mesin_id].downtimeHeaders.set(item.header_id, item.total_downtime_detik || 0);
      }
    });
    const topMachines = Object.entries(machineMap)
      .map(([id, data]) => {
        const downtime = Array.from(data.downtimeHeaders.values()).reduce((sum, val) => sum + val, 0);
    ...
    const criticalMachineMap: Record<string, { defects: number; downtimeHeaders: Map<string, number> }> = {};
    dateFilteredTransactions.forEach(item => {
      if (!criticalMachineMap[item.mesin_id]) {
        criticalMachineMap[item.mesin_id] = { defects: 0, downtimeHeaders: new Map() };
      }
      if (item.status_qc === "Recheck" && item.is_production) {
        criticalMachineMap[item.mesin_id].defects += 1;
      }
      if (item.header_id) {
        criticalMachineMap[item.mesin_id].downtimeHeaders.set(item.header_id, item.total_downtime_detik || 0);
      }
    });
  ...
  const attendanceStats = useMemo(() => {
    ...
    if (dateRangeMode !== "ALL") {
      ...
      dateScoped = dateScoped.filter(item => {
        const itemDate = new Date(item.tanggal);
        if (isNaN(itemDate.getTime())) return true;
        if (dateRangeMode === "TODAY") return itemDate.toLocaleDateString("en-CA") === todayStr;
        if (dateRangeMode === "7DAYS") {
          const sevenDaysAgo = new Date();
          sevenDaysAgo.setDate(now.getDate() - 7);
          return itemDate >= sevenDaysAgo && itemDate <= now;
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Welcome Header */}
      <div className="bg-white border border-[#e9ecef] rounded-[24px] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.015)] relative z-10">
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {dashboardViewType === "OEE" ? (
              <>
                ...
                <div
                  className="relative overflow-hidden rounded-[24px] h-full min-h-[11rem] bg-[#004777] shadow-xl ring-2 ring-[#0070bc] ring-offset-2 text-white p-5 flex flex-col justify-between transition-all duration-300 group"
                >
                  ...
                  <div className="mt-2 relative z-10">
                    ...
                    <div className="mt-2.5 flex items-center justify-between border-t border-white/10 pt-2 text-[10px] text-sky-100/90 font-bold">
                      ...
                      <span className={`font-black ${deltas.oee.type === "up" ? "text-emerald-400" : deltas.oee.type === "down" ? "text-rose-400" : "text-sky-200"}`}>
                        {deltas.oee.type === "up" ? "▲ +" : deltas.oee.type === "down" ? "▼ " : ""}{deltas.oee.value}%
                      </span>
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="mt-2 relative z-10">
                    ...
                    <div className="flex items-center gap-1.5 mt-1 text-[11px] font-extrabold text-slate-500">
                      <span>Downtime: {formatDuration(stats.totalDowntimeDetik)}</span>
                    </div>
                    <div className="mt-3 border-t border-slate-100 pt-2 flex items-center justify-between">
                      <span className="text-[10px] text-slate-400 font-bold">Tren:</span>
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="mt-2 relative z-10">
                        <div className="text-3xl font-black tracking-tight text-slate-800">{stats.performance.toFixed(1)}%</div>
                        <div className="flex items-center gap-1 mt-1 text-[11px] text-[#0070bc] font-bold">
                          <span>{stats.totalHasilPcs.toLocaleString()} / {stats.totalTarget.toLocaleString()} Panel</span>
                        </div>
                        <div className="mt-3 border-t border-slate-100 pt-2 flex items-center justify-between">
                          <span className="text-[10px] text-slate-400 font-bold">Tren:</span>
                    ...
                    >
                      ...
                      <div className="mt-2 relative z-10">
                        <div className="text-3xl font-black tracking-tight text-slate-800">{stats.totalProduksiMeter.toLocaleString()} m</div>
                        <div className="flex items-center gap-1 mt-1 text-[11px] text-[#0070bc] font-bold">
                  ...
                  <div className="absolute bottom-3 right-3 flex items-center gap-1 z-20" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      title="Geser ke PCS"
                    >
                      <div className={`h-1.5 rounded-full transition-all duration-300 ${metricMode === "PCS" ? "w-4 bg-slate-800" : "w-1.5 bg-slate-300"}`} />
                    </button>
                    <button
                      onClick={() => { setMetricMode("METER"); }}
                    ...
                    >
                      <div className={`h-1.5 rounded-full transition-all duration-300 ${metricMode === "METER" ? "w-4 bg-slate-800" : "w-1.5 bg-slate-300"}`} />
                    </button>
                  </div>
                </div>
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="mt-2 relative z-10">
                        <div className="text-3xl font-black tracking-tight text-slate-800">{stats.fpyPanel.toFixed(1)}%</div>
                        <div className="flex items-center gap-1 mt-1 text-[11px] text-[#0070bc] font-bold">
                          <span>Cacat: {stats.countMasalahPanel} dari {stats.totalPanelValid} panel</span>
                        </div>
                        <div className="mt-3 border-t border-slate-100 pt-2 flex items-center justify-between">
                          <span className="text-[10px] text-slate-400 font-bold">Tren:</span>
                    ...
                    >
                      ...
                      <div className="mt-2 relative z-10">
                        <div className="text-3xl font-black tracking-tight text-slate-800">{stats.fpyMeteran.toFixed(1)}%</div>
                        <div className="flex items-center gap-1 mt-1 text-[11px] text-[#0070bc] font-bold">
                          <span>Cacat: {stats.countMasalahMeteran} titik / {stats.totalProduksiMeter.toFixed(0)}m</span>
                        </div>
                        <div className="mt-3 border-t border-slate-100 pt-2 flex items-center justify-between">
                          <span className="text-[10px] text-slate-400 font-bold">Tren:</span>
                          <DeltaBadge delta={deltas.quality} />
                ...
                >
                  ...
                  <div className="absolute bottom-3 right-3 flex items-center gap-1 z-20" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      title="Geser ke PCS"
                    >
                      <div className={`h-1.5 rounded-full transition-all duration-300 ${metricMode === "PCS" ? "w-4 bg-white" : "w-1.5 bg-sky-350"}`} />
                    </button>
                    <button
                      onClick={() => { setMetricMode("METER"); }}
                      className="p-3 -m-3 cursor-pointer ml-3"
                      title="Geser ke METER"
                    >
                      <div className={`h-1.5 rounded-full transition-all duration-300 ${metricMode === "METER" ? "w-4 bg-white" : "w-1.5 bg-sky-350"}`} />
                    </button>
                  </div>
                </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="mt-2 relative z-10">
                        ...
                        <div className="flex items-center gap-1 mt-1 text-[11px] text-[#0070bc] font-bold">
                          <TrendingUp className="w-3.5 h-3.5" />
                          <span>{stats.countMasalahPanel} panel cacat dari {stats.totalPanelValid} panel</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className={`flex items-center gap-1.5 mt-1 text-[11px] font-extrabold ${stats.totalDowntimeDetik > 0 ? "text-amber-600" : "text-emerald-600"}`}>
                      {stats.totalDowntimeDetik > 0 ? (
                        <>
                          ...
                          <span>Downtime: {formatDuration(stats.totalDowntimeDetik)} / Total: {formatTotalHours(stats.totalDetikTersedia)}</span>
                        </>
                      ) : (
                        <>
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                          <span>Downtime: 0s / Total: {formatTotalHours(stats.totalDetikTersedia)}</span>
                        </>
                      )}
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex-1 overflow-x-auto overflow-y-hidden custom-scrollbar pb-4 w-full flex flex-col">
                      <div style={{ minWidth: `${svgWidth}px` }} className="relative mx-auto w-full flex-1 min-h-[250px]">
                        <svg viewBox={`0 0 ${svgWidth} 250`} className="absolute inset-0 w-full h-full overflow-visible" preserveAspectRatio="none">
                          ...
                          <line x1="40" y1="63.75" x2={svgWidth - 20} y2="63.75" stroke="#f1f3f5" strokeWidth="1" />
                          <line x1="40" y1="107.5" x2={svgWidth - 20} y2="107.5" stroke="#f1f3f5" strokeWidth="1" />
                          <line x1="40" y1="151.25" x2={svgWidth - 20} y2="151.25" stroke="#f1f3f5" strokeWidth="1" />
                          ...
                          <text x="30" y="67.75" fill="#a1a1aa" fontSize="9" fontWeight="bold" textAnchor="end">{Math.round(maxChartValue * 0.75)}</text>
                          <text x="30" y="111.5" fill="#a1a1aa" fontSize="9" fontWeight="bold" textAnchor="end">{Math.round(maxChartValue * 0.5)}</text>
                          <text x="30" y="155.25" fill="#a1a1aa" fontSize="9" fontWeight="bold" textAnchor="end">{Math.round(maxChartValue * 0.25)}</text>
                              ...
                              {chartGradeFilter === "ALL" && (
                                <polyline
                                  points={chartData.map((d, index) => {
                                    ...
                                    const h = maxChartValue > 0 ? (d.total / maxChartValue) * 175 : 0;
                              ...
                              {chartGradeFilter === "GRADE_A" && (
                                <polyline
                                  points={chartData.map((d, index) => {
                                    const spacing = totalWidth / Math.max(chartData.length, 1);
                                    const cx = 40 + (spacing * index) + (spacing / 2);
                                    const h = maxChartValue > 0 ? (d.gradeA_sum / maxChartValue) * 175 : 0;
                                    return `${cx},${195 - Math.max(h, 1.5)}`;
                                  }).join(' ')}
                              ...
                              {chartGradeFilter === "GRADE_B" && (
                                <polyline
                                  points={chartData.map((d, index) => {
                                    const spacing = totalWidth / Math.max(chartData.length, 1);
                                    const cx = 40 + (spacing * index) + (spacing / 2);
                                    const h = maxChartValue > 0 ? (d.gradeB_sum / maxChartValue) * 175 : 0;
                                    return `${cx},${195 - Math.max(h, 1.5)}`;
                                  }).join(' ')}
                              ...
                              {chartGradeFilter === "BS" && (
                                <polyline
                                  points={chartData.map((d, index) => {
                                    const spacing = totalWidth / Math.max(chartData.length, 1);
                                    const cx = 40 + (spacing * index) + (spacing / 2);
                                    const h = maxChartValue > 0 ? (d.bs_sum / maxChartValue) * 175 : 0;
                                    return `${cx},${195 - Math.max(h, 1.5)}`;
                                  }).join(' ')}
                              ...
                              {chartGradeFilter === "ALL" && (
                                <polyline
                                  points={chartData.map((d, index) => {
                                    const spacing = totalWidth / Math.max(chartData.length, 1);
                                    const cx = 40 + (spacing * index) + (spacing / 2);
                                    const h = maxChartValue > 0 ? (d.ungraded_sum / maxChartValue) * 175 : 0;
                                    return `${cx},${195 - Math.max(h, 1.5)}`;
                                  }).join(' ')}
                                  fill="none" stroke="#94a3b8" strokeWidth="2" strokeDasharray="4 4" strokeLinecap="round" strokeLinejoin="round"
                                />
                              )}
                          ...
                          {chartData.map((d, index) => {
                            ...
                            if (chartGradeFilter === "ALL") {
                              if (activeFilter === "PROBLEMS") {
                                const hTotal = maxChartValue > 0 ? (d.total / maxChartValue) * 175 : 0;
                              ...
                              const hVal = maxChartValue > 0 ? (value / maxChartValue) * 175 : 0;
            ...
            {(() => {
              ...
              if (metricMode === "PCS") {
                totalA = new Set(filteredData.filter(i => i.grade === "GRADE A").map(i => i.header_id)).size;
                totalB = new Set(filteredData.filter(i => i.grade === "GRADE B").map(i => i.header_id)).size;
                totalBS = new Set(filteredData.filter(i => i.grade === "BS").map(i => i.header_id)).size;
                totalUngraded = new Set(filteredData.filter(i => i.grade === "UNGRADED" || !["GRADE A", "GRADE B", "BS"].includes(i.grade)).map(i => i.header_id)).size;
              } else {
                const uniqueHeadersMap = new Map<string, typeof filteredData[0]>();
                filteredData.forEach(item => {
                  if (item.header_id && !uniqueHeadersMap.has(item.header_id)) {
                    uniqueHeadersMap.set(item.header_id, item);
                  }
                ...
                const uniqueItems = Array.from(uniqueHeadersMap.values());
                totalA = uniqueItems.filter(i => i.grade === "GRADE A").reduce((acc, curr) => acc + (parseFloat(curr.hasil_meter as any) || 0), 0);
                totalB = uniqueItems.filter(i => i.grade === "GRADE B").reduce((acc, curr) => acc + (parseFloat(curr.hasil_meter as any) || 0), 0);
                totalBS = uniqueItems.filter(i => i.grade === "BS").reduce((acc, curr) => acc + (parseFloat(curr.hasil_meter as any) || 0), 0);
                totalUngraded = uniqueItems.filter(i => i.grade === "UNGRADED" || !["GRADE A", "GRADE B", "BS"].includes(i.grade)).reduce((acc, curr) => acc + (parseFloat(curr.hasil_meter as any) || 0), 0);
              }
                    ...
                    <div className="bg-white border border-[#e9ecef] rounded-[32px] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.02)] flex flex-col justify-between flex-1">
                      <div>
                        <div className="border-b border-slate-100 pb-4 mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                          <div>
                            <h3 className="text-base font-extrabold text-slate-800">Analisis Pareto Masalah</h3>
                            <p className="text-[11px] text-slate-400 font-semibold">Distribusi kontribusi defect / downtime</p>
                          </div>
                        ...
                        <div className="relative w-full h-48 mx-auto flex items-center justify-center my-4">
                          {paretoProblemData.list.length > 0 ? (
                            (() => {
                              ...
                              const pointsStr = list.map((item, idx) => {
                              ...
                              return (
                                <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="w-full h-full overflow-visible">
                                  ...
                                  <line x1={paddingLeft} y1={paddingTop + chartHeight / 2} x2={svgWidth - paddingRight} y2={paddingTop + chartHeight / 2} stroke="#f1f5f9" strokeWidth="1" />
                                  ...
                                  {/* Left Y Axis (Values) */}
                                  <text x={paddingLeft - 6} y={paddingTop + 3} textAnchor="end" fontSize="7" fontWeight="bold" fill="#94a3b8">
                                    {paretoMode === "COUNT" ? maxValue : `${Math.round(maxValue / 60)}m`}
                                  </text>
                                  <text x={paddingLeft - 6} y={paddingTop + chartHeight / 2 + 3} textAnchor="end" fontSize="7" fontWeight="bold" fill="#94a3b8">
                                    {paretoMode === "COUNT" ? Math.round(maxValue / 2) : `${Math.round(maxValue / 120)}m`}
                                  </text>
                                  <text x={paddingLeft - 6} y={paddingTop + chartHeight + 3} textAnchor="end" fontSize="7" fontWeight="bold" fill="#94a3b8">0</text>

                                  {/* Right Y Axis (Cumulative %) */}
                                  <text x={svgWidth - paddingRight + 6} y={paddingTop + 3} textAnchor="start" fontSize="7" fontWeight="bold" fill="#ef4444">100%</text>
                                  <text x={svgWidth - paddingRight + 6} y={paddingTop + chartHeight / 2 + 3} textAnchor="start" fontSize="7" fontWeight="bold" fill="#ef4444">50%</text>
                                  <text x={svgWidth - paddingRight + 6} y={paddingTop + chartHeight + 3} textAnchor="start" fontSize="7" fontWeight="bold" fill="#ef4444">0%</text>
                                  ...
                                  {list.map((item, idx) => {
                                    ...
                                    const colors = ["#ef4444", "#f59e0b", "#3b82f6", "#8b5cf6", "#ec4899", "#10b981", "#64748b"];
                                    return (
                                      <g key={item.name} className="group/pareto-bar cursor-pointer">
                                        <rect x={x} y={y} width={barWidth} height={barHeight} rx="2" fill={colors[idx % colors.length]} opacity="0.8" className="transition-all hover:opacity-100" />
                                        <text x={x + barWidth / 2} y={y - 3} textAnchor="middle" fontSize="6" fontWeight="bold" fill="#475569" className="opacity-0 group-hover/pareto-bar:opacity-100 transition-opacity">
                                  ...
                                  {list.length > 0 && (
                                    <>
                                      <polyline points={pointsStr} fill="none" stroke="#ef4444" strokeWidth="1.5" strokeDasharray="1" />
                                      {list.map((item, idx) => {
                                        const x = paddingLeft + (barSpacing * idx) + (barSpacing / 2);
                                        const y = paddingTop + chartHeight - (item.cumulativePct / 100) * chartHeight;
                                        return (
                                          <g key={`dot-${item.name}`} className="group/dot cursor-pointer">
                                            <circle cx={x} cy={y} r="2.5" fill="#ffffff" stroke="#ef4444" strokeWidth="1.5" />
                                            <text x={x} y={y - 6} textAnchor="middle" fontSize="6" fontWeight="black" fill="#ef4444" className="opacity-0 group-hover/dot:opacity-100 transition-opacity bg-white">
                                              {item.cumulativePct}%
                                            </text>
                                          </g>
                                        );
                            ...
                            <div className="text-center py-8 text-xs text-slate-400 font-bold">Tidak ada data masalah</div>
                          )}
                        </div>

                        {/* Breakdown List */}
                        <div className="mt-6 space-y-2">
                          {paretoProblemData.list.slice(0, showAllCategories ? undefined : 3).map((item, idx) => (
                            <div key={item.name} className="flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                              <div className="flex items-center gap-3">
                                <div className="w-3.5 h-3.5 rounded-full shadow-[0_0_8px_rgba(0,0,0,0.15)]" style={{ backgroundColor: COLORS[idx % COLORS.length] }} />
                                <div className="flex flex-col">
                                  <span className="text-xs font-extrabold text-slate-700 truncate max-w-[110px]" title={item.name}>{item.name}</span>
                                  <span className="text-[8px] text-slate-400 font-semibold">Kumulatif: {item.cumulativePct}%</span>
                                </div>
                              </div>
                              <div className="text-right flex items-baseline gap-1.5">
                                {paretoMode === "COUNT" ? (
                                  ...
                                  <>
                                    <span className="text-sm font-black text-slate-800">{Math.round(item.downtime / 60)}</span>
                                    <span className="text-[10px] text-slate-400 font-bold">menit</span>
                                  </>
                                )}
                              </div>
                            </div>
                          ...
                          {!showAllCategories && paretoProblemData.list.length > 3 && (
                            <button 
                              onClick={() => setShowAllCategories(true)}
                              className="w-full mt-2 text-[11px] font-bold text-[#0070bc] hover:text-[#004777] py-2 bg-sky-50 rounded-xl transition-colors text-center"
                            >
                              Lihat Selengkapnya ({paretoProblemData.list.length - 3} lainnya)
                            </button>
                          )}
                    ...
                    <div>
                      <div className="border-b border-slate-100 pb-4 mb-6 flex justify-between items-start">
                        ...
                        <span className="text-[9px] font-bold px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 flex items-center gap-1 shrink-0">
                          {totalQuality.toLocaleString()} {metricMode === "PCS" ? "Panel" : "Baris"}
                        </span>
                      </div>
                      ...
                      <div className="mt-8 space-y-2">
                        <div className="flex items-center justify-between p-3 rounded-2xl hover:bg-sky-50/50 transition-colors border border-transparent hover:border-sky-100/50">
                          ...
                          <div className="text-right">
                            ...
                            <span className="text-[10px] text-slate-400 font-bold ml-1.5">{metricMode === "PCS" ? "Panel" : "Baris"}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50/50 transition-colors border border-transparent hover:border-slate-100/50">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-extrabold text-slate-700">Belum Diinspeksi (Ungraded)</span>
                          </div>
                          <div className="text-right">
                            <span className="text-sm font-black text-slate-800">{totalUngraded.toLocaleString()}</span>
                            <span className="text-[10px] text-slate-400 font-bold ml-1.5">{metricMode === "PCS" ? "Panel" : "Baris"}</span>
                          </div>
                        </div>
                      </div>
                ...
                <div>
                  <div className="border-b border-slate-100 pb-3 mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-extrabold text-slate-800">Operator Terbaik (Top 5)</h3>
                    </div>
                  ...
                  <div className="space-y-2">
                    {leaderboard.topOperators.length > 0 ? (
                      leaderboard.topOperators.map((op, idx) => (
                        <div key={op.name} className="flex items-center justify-between p-2.5 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-colors">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              <span className="text-xs font-bold text-slate-700 truncate max-w-[120px]">{op.name}</span>
                              <span className="text-[9px] font-medium text-slate-400">{op.group}</span>
                            </div>
                          </div>
                          <span className="text-xs font-black text-slate-800">{op.pcs.toLocaleString()} panel</span>
                        </div>
                      ))
                    ) : (
                      <div className="text-center py-6 text-xs text-slate-400 font-bold">Tidak ada data</div>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div>
                  <div className="border-b border-slate-100 pb-3 mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-extrabold text-slate-800">Mesin Terproduktif (Top 5)</h3>
                    </div>
                  ...
                  <div className="space-y-2">
                    {leaderboard.topMachines.length > 0 ? (
                      leaderboard.topMachines.map((m, idx) => (
                        <div key={m.id} className="flex items-center justify-between p-2.5 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-colors">
                          <div className="flex items-center gap-3">
                            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black ${
                              idx === 0 ? "bg-sky-100 text-sky-700" : "bg-slate-100 text-slate-500"
                            }`}>
                              {idx + 1}
                            </span>
                            <span className="text-xs font-bold text-slate-700">{m.id}</span>
                          </div>
                          <div className="text-right">
                            <span className="text-xs font-black text-slate-800">{m.pcs.toLocaleString()} panel</span>
                          </div>
                        </div>
                      ))
                    ) : (
                ...
                <div>
                  <div className="border-b border-slate-100 pb-3 mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-extrabold text-slate-800">Perlu Inspeksi (Bottom 5)</h3>
                    </div>
                  ...
                  <div className="space-y-2">
                    {leaderboard.criticalMachines.length > 0 ? (
                      leaderboard.criticalMachines.map((m, idx) => (
                        <div key={m.id} className="flex items-center justify-between p-2.5 rounded-xl hover:bg-red-55/20 border border-transparent hover:border-red-100/30 transition-colors">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-bold text-red-700">{m.id}</span>
                          </div>
                          <div className="text-right flex flex-col items-end">
                            <span className="text-xs font-black text-rose-600">{m.defects} cacat</span>
                            {m.downtime > 0 && (
                      ...
                      <div className="text-center py-6 text-xs text-emerald-600 font-extrabold flex flex-col items-center gap-1">
                        ...
                        <span className="text-[9px] text-slate-400 font-medium">0 record cacat/downtime</span>
                      </div>
                    )}
                  </div>
                </div>
